...
const Providers = () => {
  ...
  return (
    <div className="p-6 relative">
      <h2 className="text-2xl font-bold text-blue-800 dark:text-blue-400 mb-4">Service Providers</h2>
          ...
          <tbody>
            {filteredProviders.map((p, i) => (
              <tr key={i} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td className="p-3">
                  <span className={`px-2 py-1 text-xs rounded-full font-semibold ${getStatusColor(p.status)}`}>
                    {p.status}
                  </span>
                </td>
      ...
      {selectedProvider && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-3">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full sm:w-[450px] p-6 relative animate-fadeIn">
            <button onClick={() => setSelectedProvider(null)} className="absolute top-3 right-3 text-gray-500 hover:text-red-500">
              <X size={20} />
            </button>
            ...
            <div className="space-y-2 text-sm">
              ...
              <p><strong>Category:</strong> {selectedProvider.category}</p>
              <p><strong>Jobs:</strong> {selectedProvider.jobs}</p>
