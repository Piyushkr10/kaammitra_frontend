...
import { User, Phone, MapPin, Moon } from "lucide-react";
...
const OtpVerification = ({ phoneNumber, onVerify }) => {
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md bg-gray-400 ">
      <h2 className="text-xl font-bold text-gray-800 mb-2 text-center text-white">Verify Your Phone</h2>
...
const AddressForm = ({ onAddressSubmit }) => {
  ...
  return (
    <div className=" p-8 rounded-xl shadow-lg w-full max-w-md bg-gray-400">
      <h2 className="text-xl font-bold text-white mb-6 text-center">Address Details</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="address" className="block mb-1 font-medium text-white">Address</label>
          <input
            type="text"
            id="address"
...
export default function SignUp({ onRegistrationComplete }) {
  ...
  const handleAddressSubmit = (addressData) => {
    console.log("Final registration data:", { fullName, phoneNumber, ...addressData });
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-4">
      ...
      <div className="flex items-center justify-between w-full max-w-sm mb-12">
        <div className={`w-10 h-10 rounded-full ${step >= 1 ? 'bg-blue-600 text-white' : 'border border-gray-400 text-gray-500'} flex items-center justify-center`}>1</div>
        <div className={`flex-1 h-1 ${step > 1 ? 'bg-blue-600' : 'bg-gray-300'}`}></div>
        <div className={`w-10 h-10 rounded-full ${step >= 2 ? 'bg-blue-600 text-white' : 'border border-gray-400 text-gray-500'} flex items-center justify-center`}>2</div>
        <div className={`flex-1 h-1 ${step > 2 ? 'bg-blue-600' : 'bg-gray-300'}`}></div>
        <div className={`w-10 h-10 rounded-full ${step >= 3 ? 'bg-blue-600 text-white' : 'border border-gray-400 text-gray-500'} flex items-center justify-center`}>3</div>
      </div>

      {step === 1 && (
        <div className="bg-gray-400 p-8 rounded-xl shadow-lg w-full max-w-md">
          <h2 className="text-xl font-bold mb-6 text-center text-white">Personal Information</h2>
      ...
      {step === 2 && <OtpVerification phoneNumber={phoneNumber} onVerify={handleVerificationSuccess} />}
      {step === 3 && <AddressForm onAddressSubmit={handleAddressSubmit} />}
    </div>
  );
}